...
**聚焦期间的按键**：一旦有输入框聚焦，**所有按键事件都会被 `TextInputGate` 消费**（导航/编辑键交给输入框处理，其余按键丢弃），不再进入全局 `Input`——打字时不会误触发绑定到 WASD/空格等键的游戏动作。按 `Esc` 释放焦点后按键恢复进入全局输入。按键抬起也按聚焦时按下的集合配对消费，不会让全局输入卡键。
